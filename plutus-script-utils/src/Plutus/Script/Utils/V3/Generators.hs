...
forwardingMintingPurpose
  :: (ToScriptHash script) => script -> (mtx -> [TxInInfo]) -> MintingPurposeType' red mtx
...
forwardingSpendingPurpose
  :: (ToScriptHash script) => script -> (txInfo -> MintValue) -> SpendingPurposeType' dat red txInfo
...
ownForwardingSpendingPurpose
  :: (txInfo -> MintValue) -> (txInfo -> [TxInInfo]) -> SpendingPurposeType' dat red txInfo
ownForwardingSpendingPurpose toMintValue toTxInInfos oRef dat red txInfo =
  case [ hash | TxInInfo ref (TxOut (Address (ScriptCredential hash) _) _ _ _) <- toTxInInfos txInfo, ref == oRef
       ] of
